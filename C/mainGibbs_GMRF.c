 ...
 *			shapeEps, scaleEps: scale and shape parameters for inverse-gamma prior of sigEps
 *			shapeBeta, scaleBeta: the same for sigBeta
 *			alphaRes, betaRes, sigEpsRes, sigBetaRes: result vectors
 * 			betaLower, betaUpper: empty vectors for pointwise quantile-based CI
 */ 
void allGibbsGMRF(int* K, int* nPat, int* nVox, int* nFixEf, int* nDelta, int* nMCMC, int* burnin, // array sizes
              ...
              double* shapeEps, double* scaleEps, double* shapeBeta, double* scaleBeta, // hyperparameters for variances
			  int* calcCI,  // should pointwise quantile CI be calculated (2.5 and 97.5 quantiles)
              double* alphaRes, double* betaRes, double* sigEpsRes, double* sigBetaRes,
			  double* betaLower, double* betaUpper, double* alphaLower, double* alphaUpper) //vectors for results 
{
	/* auxiliary variables */
	...
	//parameters: variances, set to prior mode
	sigEps = *scaleEps / (1 + *shapeEps);
	sigBeta =  *scaleBeta / (1 + *shapeBeta);
	...
	*sigBetaRes = 0;
	...
	for(l = 0; l < *nVox; l++)
	{
		...
		for(i = 0; i < *nDelta; i++)
		{
			if(matrix(delta, *nVox, l, i) !=  NA_INTEGER) // matrix(delta, nVox, l, i)-1 is neighbour of l
			{
				...
				sumPredNeigh[l] = sumPredNeigh[l] + beta[matrix(delta, *nVox, l, i)-1];
			}
		}	
	}	
	...
	for(k = 0; k < (*K); k++) // K Gibbs steps
	{
		...
		// update beta : sweep over all voxels
		for(j = 0; j < (*nVox); j++) 
		{ 		
			...
			/* do local iteration */
			updateBetaOnly(*nPat, *nVox, *nDelta, l, Xl, Y, beta, InProd, delta, xbeta0, FixEfAlpha, numNeigh, sumPredNeigh, sigEps, sigBeta);
		...
		/* update hyperparameters (informative Inverse-Gamma Priors)*/
		updateSigEpsInformative(&sigEps, *shapeEps, *scaleEps, *nPat, Y, FixEfAlpha, xbeta);

		updateSigBetaInformative_betaOnly(&sigBeta, *shapeBeta, *scaleBeta, *nVox, beta, numNeigh, sumPredNeigh);	
		...
		if(k >= *burnin)
		{
			if(k % (*nMCMC) == 0)
			{
				...
				*sigEpsRes = *sigEpsRes + sigEps;
				*sigBetaRes = *sigBetaRes + sigBeta;
	...
	*sigEpsRes = *sigEpsRes / countRes;
	*sigBetaRes = *sigBetaRes / countRes;
